...
export default function Sidebar({ params }: { params: { id: string } }) {
    ...
    return (
        <aside className={`fixed top-0 left-0 mt-header-h w-full ${(isTOCShowed) ? "h-[calc(100vh-theme(height.header-h))] bg-default" : "h-auto bg-transparent"} md:static md:px-0 md:w-auto md:h-full`}>
            ...
            <div
                className={`w-full flex-col gap-[24px] ${(isTOCShowed) ? "flex" : "hidden"} px-screen-x-default box-border h-auto max-h-[calc(100vh-64px-64px)] md:fixed md:top-[160px] md:flex md:w-[225px] md:max-h-[calc(100vh-160px-64px)]`}>
                ...
                <div onClick={() => { setIsTOCShowed(false) }}>
                    <TOC />
                </div>
            </div>
        </aside>
